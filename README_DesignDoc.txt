...
headers 
	...
	- Matthew has added commemts with his struc headers to indicate how the pointers operate and function with the design of the program.
...
readmips
	...
	- The fuction reads in the MIPS instructions and accounts for the differing amount of elements an instruction can have.
...
control logic
	...
	- The instructions are also sent to the appropricate pipelining stage that it needs to go to next.
...
decode
	- The decodeinstruction() is taking in the instruction after fetch and (depending on its type) will parse through and make the correct values within the structs to deliniate in pieces what the instruction wants. 

execute 
	- The pipeline stage consists of three execuse for the three types of instruction type.
		- The execute functions will create an appropriate exmem struct that will create a destintion where the values can be store so the memory pipeline stage can use those values.
		- Each fuction will return a stuct that will take in it's type from the decode stage
...
memory
	- The memory_rw() function takes in the specific instruction from decode.  The function will then creat an appropriate memwb struct to allocate a destination for the values produced by the function.
...
fetch helpers
	- The fucntions make_r_type(), make_i_type(), and make_j_type() are functions that take in the instructions
...
memory helpers
	- All of the Data Transfermation load functions are here.

wb helpers
	- All of the Data Transfermation store functions are here.
